...
export default function TicketSubmit({show, setShow, resourceName, resource, resourceMode, resourceData, onResourceSubmitted, orientation, type: type2}) {
    const resolver = yupResolver(yup.object().shape({
        number: yup.string().required("Поле обязательное*"),
        date: yup.string().required("Поле обязательное*"),
        isSeat: yup.string().required("Поле обязательное*"),
        ...
        <ResourceSubmit resource={resource} show={show} setShow={setShow} resourceName={resourceName} resourceMode={resourceMode} resourceData={resourceData} onResourceSubmitted={onResourceSubmitted} onSubmit={async handler => { handleSubmit(handler)(); }}>
            <HookForm methods={methods} data={resourceData}>
                <Fields>
                    ...
                    <FormField type="boolean" name="isSeat" label="IsSeat" value={isSeat} error={errors.isSeat?.message} trigger={trigger} onChange={value => setValue('isSeat', value)} isValidated={true} />
                    ...
                    <FormField type="number" name="state" label="State" value={state} error={errors.state?.message} trigger={trigger} onChange={value => setValue('state', value)} isValidated={true} />
