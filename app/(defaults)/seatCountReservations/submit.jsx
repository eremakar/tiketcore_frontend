...
export default function SeatCountReservationSubmit({show, setShow, resourceName, resource, resourceMode, resourceData, onResourceSubmitted, orientation, type: type2}) {
    const resolver = yupResolver(yup.object().shape({
        number: yup.string().required("Поле обязательное*"),
        dateTime: yup.string().required("Поле обязательное*"),
        ...
        <ResourceSubmit resource={resource} show={show} setShow={setShow} resourceName={resourceName} resourceMode={resourceMode} resourceData={resourceData} onResourceSubmitted={onResourceSubmitted} onSubmit={async handler => { handleSubmit(handler)(); }}>
            <HookForm methods={methods} data={resourceData}>
                <Fields>
                    ...
                    <FormField type="datetime" name="dateTime" label="DateTime" value={dateTime} error={errors.dateTime?.message} trigger={trigger} onChange={value => setValue('dateTime', value)} isValidated={true} />
